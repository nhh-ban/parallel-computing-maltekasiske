...
MTweedieTests <-  
  function(N, M, sig){ 
    ...
    # Split M simulations into multiple cores
    num_cores <- detectCores()
    M_per_core <- ceiling(M / num_cores)
    results <- mclapply(1:num_cores, function(i) {
      replicate(M_per_core, simTweedieTest(N))
    })
    
    # Combine the results from all cores
    all_results <- unlist(results)
    sum(all_results < sig) / M
  } 
